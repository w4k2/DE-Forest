...
from utils_diversity import calc_diversity_measures0
...
logging.basicConfig(filename='textinfo/experiment_cv.log', filemode="a", format='%(asctime)s - %(levelname)s: %(message)s', level='DEBUG')
logging.info("--------------------------------------------------------------------------------")
    ...
    try:
        ...
        for clf_id, clf_name in enumerate(methods):
            ...
            for metric_id, metric in enumerate(metrics_alias):
                ...
                if not os.path.exists("results/experiment_cv/raw_results/%s/%s/" % (metric, dataset_name)):
                    os.makedirs("results/experiment_cv/raw_results/%s/%s/" % (metric, dataset_name))
            ...
            if not os.path.exists("results/experiment_cv/diversity_results/%s/" % (dataset_name)):
                os.makedirs("results/experiment_cv/diversity_results/%s/" % (dataset_name))
