...
logging.basicConfig(filename='textinfo/pre_experiment_bac.log', filemode="a", format='%(asctime)s - %(levelname)s: %(message)s', level='DEBUG')
logging.info("--------------------------------------------------------------------------------")
    ...
    try:
        ...
        for fold_id, (train, test) in enumerate(rskf.split(X, y)):
            ...
            for clf_id, clf_name in enumerate(methods):
                ...
                filename = "results/pre_experiment_bac/optimization/%s/%s_agg_%d.csv" % (dataset_name, clf_name, fold_id)
                if not os.path.exists("results/pre_experiment_bac/optimization/%s/" % (dataset_name)):
                    os.makedirs("results/pre_experiment_bac/optimization/%s/" % (dataset_name))
        ...
        for clf_id, clf_name in enumerate(methods):
            ...
            for metric_id, metric in enumerate(metrics_alias):
                ...
                if not os.path.exists("results/pre_experiment_bac/raw_results/%s/%s/" % (metric, dataset_name)):
                    os.makedirs("results/pre_experiment_bac/raw_results/%s/%s/" % (metric, dataset_name))
                np.savetxt(fname=filename, fmt="%f", X=scores[metric_id, clf_id, :])
            # Save diversity results
            filename = "results/pre_experiment_bac/diversity_results/%s/%s_diversity.csv" % (dataset_name, clf_name)
            if not os.path.exists("results/pre_experiment_bac/diversity_results/%s/" % (dataset_name)):
                os.makedirs("results/pre_experiment_bac/diversity_results/%s/" % (dataset_name))
            ...
            filename = "results/pre_experiment_bac/diversities_cor_q_k/%s/%s_cqk.csv" % (dataset_name, clf_name)
            if not os.path.exists("results/pre_experiment_bac/diversities_cor_q_k/%s/" % (dataset_name)):
                os.makedirs("results/pre_experiment_bac/diversities_cor_q_k/%s/" % (dataset_name))
