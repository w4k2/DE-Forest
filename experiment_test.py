...
logging.basicConfig(filename='textinfo/experiment_test.log', filemode="a", format='%(asctime)s - %(levelname)s: %(message)s', level='DEBUG')
logging.info("--------------------------------------------------------------------------------")
    ...
    try:
        ...
        for clf_id, clf_name in enumerate(methods):
            ...
            for metric_id, metric in enumerate(metrics_alias):
                filename = "results/experiment_test/raw_results/%s/%s/%s.csv" % (metric, dataset_name, clf_name)
                if not os.path.exists("results/experiment_test/raw_results/%s/%s/" % (metric, dataset_name)):
                    os.makedirs("results/experiment_test/raw_results/%s/%s/" % (metric, dataset_name))
                np.savetxt(fname=filename, fmt="%f", X=scores[metric_id, clf_id, :])
            # Save diversity results
            filename = "results/experiment_test/diversity_results/%s/%s_diversity.csv" % (dataset_name, clf_name)
            if not os.path.exists("results/experiment_test/diversity_results/%s/" % (dataset_name)):
                os.makedirs("results/experiment_test/diversity_results/%s/" % (dataset_name))
